...
pub fn generate_matrix(title: &str, content: &str) -> (usize, usize, Box<[bool]>) {
    ...
    let h = title_vmetrics.ascent - title_vmetrics.descent + title_vmetrics.line_gap;
    let mut y = 0.0;
    let title_glyphs: Vec<PositionedGlyph<'_>> = font
        .glyphs_for(title.chars())
        .scan((None, 0.0), |(last, x), g| {
            let g = g.scaled(title_scale);
            if let Some(last) = last {
                *x += font.pair_kerning(title_scale, *last, g.id());
            ...
            let next = g.positioned(title_offset + vector(*x, y));
    ...
    let body_offset = point(5.0, h + body_vmetrics.ascent + y + 5.0);
    ...
    for g in title_glyphs.into_iter().chain(body_glyphs.into_iter())
    {
